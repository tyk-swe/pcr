...
use crate::rules::template::render_option;
...
impl RuleSendTemplate {
    ...
    pub fn render(&self, packet: Option<&PacketContext>) -> PacketRequest {
        ...
        if let Some(TransportProtocolRequest::Tcp(_)) = request.transport.command.as_mut() {
            // No TCP option fields are currently templated
        }
