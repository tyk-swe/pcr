...
impl transmit::Sender for NeverTransmit {
    ...
    ) -> Result<transmit::Report, packetcraftr_network::Error> {
...
impl Resolver for CountingResolver {
    fn resolve(
        ...
        _hostname: &Hostname,
        _limit: usize,
    ) -> Result<Vec<IpAddr>, packetcraftr_live::target::Error> {
        self.calls.fetch_add(1, Ordering::SeqCst);
...
fn denied_resolved_address_never_reaches_route_neighbor_or_transmit_providers() {
    ...
    let client = Client::new(
        Arc::new(
            packetcraftr_packet::protocol::builtin::registry().expect("built-ins must register"),
        ),
        CountingRoutes {
    ...
    let error = client
        .plan_target(
            ...
            &target,
            &resolver,
            &packetcraftr_network::route::Options::default(),
        )
