...
//! Audited, policy-gated packet transmission and response exchange.
//!
//! The client owns orchestration only: materialization, route selection, send,
//! capture-ready exchange, response matching, cleanup, and evidence. Every
//! authorization decision it makes is delegated to [`packetcraftr_policy`], so
//! a client cannot widen the traffic policy it was constructed with.
...
pub use packetcraftr_net::Stats;
...
impl<R: RouteProvider> RouteProvider for ExchangeRouteProvider<'_, R> {
    ...
    fn classify_error(&self, error: &Self::Error) -> packetcraftr_model::error::Classification {
