...
//!
//! This directory is the only location in the workspace permitted to contain
//! FFI or narrowly reviewed unsafe code. Public traits and values live in
//! `packetcraftr-net`; the provider types that select these adapters live in
//! this crate's sibling modules.
...
use packetcraftr_net::interface::InterfaceInfo;
#[cfg(any(
    all(
        feature = "native-layer2",
        any(target_os = "linux", target_os = "macos", windows)
...
use packetcraftr_net::route::InterfaceId;
#[cfg(all(
    feature = "native-route",
    any(target_os = "linux", target_os = "macos", windows)
))]
use packetcraftr_net::route::RouteDecision;
#[cfg(all(
    feature = "native-route",
    any(target_os = "linux", target_os = "macos", windows)
))]
use packetcraftr_net::route::{RouteSelectionReason, classify_destination};
...
fn fallback_source(
    addresses: &[packetcraftr_net::interface::InterfaceAddress],
    destination: IpAddr,
) -> Option<IpAddr> {
...
pub(super) fn interface_decision(
    ...
    Ok(RouteDecision {
        ...
        destination_scope: packetcraftr_net::route::DestinationScope::Unspecified,
        mtu,
