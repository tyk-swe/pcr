...
fn representative_response() -> packetcraftr::dns::ValidatedResponse {
    ...
    response
        .rejected_records
        .push(packetcraftr::dns::RejectedRecord {
...
fn completion_construction_rejects_incoherent_transport_or_response_metadata() {
    let metadata = representative_response().metadata;
    for (outcome, fallback, transport, header) in [
        ...
        (
            packetcraftr::dns::Outcome::Truncated,
            true,
            Some(packetcraftr::dns::Transport::Tcp),
            Some(metadata.clone()),
        ),
        (
            packetcraftr::dns::Outcome::Response,
        ...
        assert!(packetcraftr::dns::Completion::new(outcome, fallback, transport, header).is_err());
    }
}
...
fn report_construction_requires_fallback_and_attempts_to_agree() {
    ...
    for (fallback, attempts) in [(false, vec![tcp]), (true, vec![attempt_evidence()])] {
        let summary = packetcraftr::dns::Summary {
            server: "resolver.example.test".into(),
            server_port: 53,
            resolved_addresses: Vec::new(),
        ...
        assert!(
            packetcraftr::dns::Report::new(summary, None, attempts, Vec::new(), Vec::new())
                .is_err()
        );
    }
}
