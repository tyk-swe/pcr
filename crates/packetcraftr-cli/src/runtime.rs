...
use clap::Parser;
use packetcraftr_client as client;
use packetcraftr_client::Client;
use packetcraftr_net as net;
use packetcraftr_net::exchange::Composite;
use packetcraftr_net_native as net_native;
use packetcraftr_output as output;
use packetcraftr_packet as packet;
use packetcraftr_packet::Packet;
use packetcraftr_policy as policy;
use packetcraftr_protocols as protocol;
...
impl DeferredInterface {
    ...
    pub(super) fn resolve_into(
        ...
        options.interface = resolve_interface(
            Some(selector.clone()),
            &net_native::interface::SystemProvider,
        )?;
        *self = Self::Resolved;
        Ok(())
    }
...
type SystemPacketIo =
    net::transmit::Dispatch<net_native::transmit::SystemLayer2, net_native::transmit::SystemLayer3>;
type SystemExchangeIo = Composite<SystemPacketIo, net_native::capture::SystemProvider>;
pub(super) type SystemClient = Client<
    net_native::route::SystemProvider,
    net_native::neighbor::SystemResolver,
    SystemExchangeIo,
>;
...
pub(super) fn system_client(
    ...
    Client::new(
        ...
        Composite::new(
            net::transmit::Dispatch::new(
                net_native::transmit::SystemLayer2,
                net_native::transmit::SystemLayer3,
            ),
            net_native::capture::SystemProvider,
        ),
        policy,
    )
}
...
pub(super) fn parse_workflow_target(target: String) -> Result<workflow::target::Target, CliError> {
    match target
        .parse::<policy::target::Target>()
        .map_err(CliError::classified)?
    {
        policy::target::Target::Address(address) => Ok(workflow::target::Target::Address(address)),
        policy::target::Target::Hostname(hostname) => {
...
pub(super) fn prepare_route_request(
    ...
    let interface = resolve_interface(interface, &net_native::interface::SystemProvider)?;
...
fn resolve_live_destination(
    ...
    let target = destination
        ...
        .map_err(CliError::classified)?;
    let resolved = policy
        .resolve_target(&target, &policy::target::SystemResolver)
        .map_err(CliError::classified)?;
    let ip_version = packet
        .iter()
        .find_map(|layer| match layer.protocol_id().as_str() {
            "ipv4" => Some(policy::target::IpVersion::V4),
            "ipv6" => Some(policy::target::IpVersion::V6),
            _ => None,
        });
    match ip_version {
        Some(version) => resolved
            ...
            .map(Some)
            .ok_or_else(|| {
                CliError::classified(policy::target::Error::AddressFamilyUnavailable {
