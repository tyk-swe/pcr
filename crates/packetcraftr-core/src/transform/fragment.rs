...
pub fn fragment(frame: &Frame, options: FragmentOptions) -> Result<Vec<Frame>, Error> {
    ...
    if frame.captured_length() != frame.original_length() {
        return Err(Error::Invalid(InvalidInput::FragmentTruncatedCapture));
    }
    let link = LinkHeader::walk(frame.link_type, frame.bytes())?
        .ok_or(Error::Unsupported(Unsupported::LinkType))?;
    let header = link
        .walk_ip(frame.bytes())?
        .ok_or(Error::Unsupported(Unsupported::EthernetPayload))?;
...
fn ipv4(
    ...
    if checksum(&ip[..header_len]) != 0 {
        return Err(Error::Invalid(InvalidInput::Ipv4HeaderChecksum));
    }
    if ipv4.reserved_flag() {
        return Err(Error::Invalid(InvalidInput::Ipv4ReservedFlag));
    }
    if ipv4.is_fragment() {
        return Err(Error::Unsupported(Unsupported::AlreadyFragmented));
    }
    if length <= options.mtu {
        return unchanged(frame, options);
    }
    if ipv4.dont_fragment() {
        return Err(Error::Unsupported(Unsupported::DontFragment));
    }
    if length == header_len {
        return Err(Error::Invalid(InvalidInput::MtuBelowIpv4Header));
    }
    ...
    let id = options
        ...
        .map(u16::try_from)
        .transpose()
        .map_err(|_| Error::Invalid(InvalidInput::Ipv4Identification))?
    ...
    while position < payload.len() {
        ...
        let room = options
            ...
            .ok_or(Error::Invalid(InvalidInput::MtuBelowIpv4Header))?;
        ...
        let total = u16::try_from(header.len() + count)
            .map_err(|_| Error::Invalid(InvalidInput::Ipv4FragmentLength))?;
...
fn ipv6(
    ...
    if ipv6.payload_length() == 0 {
        return Err(Error::Unsupported(Unsupported::Ipv6PayloadLength));
    }
    ...
    for (index, extension) in ipv6.extensions().iter().enumerate() {
        match extension.protocol() {
            ip_protocol::FRAGMENT | ip_protocol::AH => {
                return Err(Error::Unsupported(Unsupported::Ipv6FragmentOrIpsec));
            }
            ip_protocol::HOP_BY_HOP if index != 0 => {
                return Err(Error::Unsupported(Unsupported::MisorderedHopByHop));
            }
    ...
    let transport_len = match next {
        ip_protocol::TCP => {
            let value = *ip
                .get(cursor + 12)
                .ok_or(Error::Invalid(InvalidInput::TruncatedTcpHeader))?;
    ...
    if cursor + transport_len > length {
        return Err(Error::Invalid(InvalidInput::TruncatedUpperLayer));
    }
    let room = options
        .mtu
        .checked_sub(prefix_len + 8)
        .ok_or(Error::Invalid(InvalidInput::MtuBelowIpv6Headers))?;
    if room / 8 * 8 < cursor + transport_len - prefix_len {
        return Err(Error::Invalid(InvalidInput::Ipv6FirstFragment));
    }
    let id = options
        .identification
        .ok_or(Error::Invalid(InvalidInput::Ipv6Identification))?;
    ...
    while position < payload.len() {
        ...
        let payload_length = u16::try_from(prefix_len - 40 + 8 + count)
            .map_err(|_| Error::Invalid(InvalidInput::Ipv6FragmentLength))?;
...
fn payload_count(room: usize, remaining: usize) -> Result<usize, Error> {
    ...
    if count == 0 {
        return Err(Error::Invalid(InvalidInput::MtuFragmentPayload));
    }
    Ok(count)
}
fn append(
