...
const NAME: &str = BuiltinProtocol::Ipv6Srh.as_str();
...
reflective_layer! {
    fn srh_schema() => { protocol: protocol(NAME), name: "IPv6 Segment Routing Header" }
...
impl LayerCodec for SegmentRoutingHeaderCodec {
    ...
    fn encode(
        ...
        let layer = typed_layer::<SegmentRoutingHeader>(NAME, layer)?;
        ...
        Ok(EncodedLayer::header(prefix, Box::new(materialized))
            .with_fields(srh_layout(segments_end, header_len))
    ...
    fn decode(
        ...
        let final_destination = wire_segments
            .last()
            .copied()
            .ok_or_else(|| invalid(NAME, "segment list is empty"))?;
        let tlv_bytes = input
...
fn srh_lengths(layer: &SegmentRoutingHeader) -> Result<(usize, usize), crate::codec::Error> {
    let segments_end = layer
        ...
        .checked_mul(16)
        .and_then(|length| length.checked_add(8))
        .ok_or_else(|| invalid(NAME, "SRH segment-list length overflow"))?;
    let unpadded_len = segments_end
        .checked_add(layer.tlvs.len())
        .ok_or_else(|| invalid(NAME, "SRH TLV length overflow"))?;
    let header_len = unpadded_len
        .checked_next_multiple_of(8)
        .ok_or_else(|| invalid(NAME, "SRH padding overflow"))?;
