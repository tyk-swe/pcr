...
mod tests {
    ...
    #[test]
    fn plans_mtu_fragments_on_eight_byte_boundaries() {
        let frag = FragmentSpec {
            mtu: Some(36),
            ..Default::default()
        };

        let fragments = plan_fragments(&frag, 40, 20, 0).unwrap();

        assert_eq!(fragments.len(), 3);
        assert_eq!(
            fragments
                .iter()
                .map(|fragment| (fragment.start, fragment.len, fragment.more))
                .collect::<Vec<_>>(),
            vec![(0, 16, true), (16, 16, true), (32, 8, false)]
        );
        for fragment in &fragments {
            ensure_fragment_alignment(fragment).unwrap();
        }
    }

    #[test]
    fn rejects_misaligned_fragment_offsets() {
        ...
        assert!(matches!(
            ...
            Err(FragmentError::Misaligned { offset: 4 })
        ));
    }
}
