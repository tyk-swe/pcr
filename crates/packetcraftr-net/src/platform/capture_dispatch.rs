...
mod tests {
    ...
    #[test]
    fn resolver_free_capture_filter_accepts_numeric_operands_and_rejects_names() {
        for source in [
            ...
            assert!(
                validate_resolver_free_capture_filter(&interface().id, source).is_ok(),
                "{source}"
            );
        }
        for source in [
            ...
            let error =
                validate_resolver_free_capture_filter(&interface().id, source).expect_err(source);
    ...
    #[test]
    fn capture_netmask_prefers_the_selected_ipv4_prefix() {
        let interface = interface();
        assert_eq!(
            capture_netmask(Some(IpAddr::V4(Ipv4Addr::new(198, 51, 100, 7))), &interface),
            Some(u32::from_ne_bytes([255, 255, 255, 128]))
        );
        assert_eq!(
            capture_netmask(Some(IpAddr::V6(std::net::Ipv6Addr::LOCALHOST)), &interface),
            Some(u32::from_ne_bytes([255, 255, 255, 0]))
        );
        let mut without_ipv4 = interface;
        without_ipv4.addresses.clear();
        assert_eq!(capture_netmask(None, &without_ipv4), None);
    }
}
