...
use packetcraftr_model::{FrameError, LinkType};

/// Default maximum size of an offline packet or a PCAPNG block (16 MiB).
pub const DEFAULT_SIZE_LIMIT: usize = packetcraftr_model::DEFAULT_MAX_FRAME_BYTES;
...
pub enum Error {
    ...
    /// A frame length invariant owned by the shared frame model.
    #[error(transparent)]
