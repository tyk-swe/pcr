...
impl TcpReassembler {
    ...
    pub fn open_flow(
        ...
        if self.flows.len() >= self.limits.max_flows {
            return Err(TcpReassemblyError::FlowLimit {
    ...
    pub fn push(
        ...
        segment: TcpSegment,
        now: Instant,
    ) -> Result<Vec<TcpReassemblyEvent>, TcpReassemblyError> {
        ...
        let mut offset =
            u64::try_from(absolute.max(0)).map_err(|_| TcpReassemblyError::FlowByteLimit {
                limit: self.limits.max_bytes_per_flow,
            })?;
        let fin_offset =
            offset
                .checked_add(payload.len() as u64)
                .ok_or(TcpReassemblyError::FlowByteLimit {
                    limit: self.limits.max_bytes_per_flow,
                })?;
        ...
        if let Some(final_offset) = candidate.fin_offset {
            if segment.fin && fin_offset != final_offset {
                return Err(TcpReassemblyError::ConflictingFinalSequence {
                    existing_offset: final_offset,
                    new_offset: fin_offset,
                });
            }
            if remaining_end > final_offset {
                return Err(TcpReassemblyError::BeyondFinalSequence { final_offset });
            }
        }
        if segment.fin && candidate.next_offset > fin_offset {
            return Err(TcpReassemblyError::BeyondFinalSequence {
                final_offset: fin_offset,
            });
        }
        if offset > window_end || remaining_end > window_end {
            return Err(TcpReassemblyError::FlowByteLimit {
                limit: self.limits.max_bytes_per_flow,
            });
        }

        let old_retained_bytes =
            retained_bytes(&candidate).ok_or(TcpReassemblyError::AggregateByteLimit {
                limit: self.limits.max_aggregate_bytes,
            })?;
        let old_memory_charge =
            flow_memory_charge(&candidate).ok_or(TcpReassemblyError::AggregateByteLimit {
                limit: self.limits.max_aggregate_bytes,
            })?;
        let (pending, pending_bytes, overlap, pending_conflicting) =
            merge_pending(&candidate.pending, offset, payload).ok_or(
                TcpReassemblyError::FlowByteLimit {
                    limit: self.limits.max_bytes_per_flow,
                },
            )?;
        ...
        if pending.len() > self.limits.max_tcp_segments_per_flow {
            return Err(TcpReassemblyError::SegmentLimit {
        ...
        if retransmitted != 0 || conflicting {
            events.push(TcpReassemblyEvent::Retransmission {
        ...
        loop {
            ...
            events.push(TcpReassemblyEvent::Data {
                flow: segment.flow.clone(),
                sequence,
                bytes: output,
            });
        ...
        let aggregate_bytes = self
            ...
            .ok_or(TcpReassemblyError::AggregateByteLimit {
                limit: self.limits.max_aggregate_bytes,
            })?;
        ...
        {
            return Err(TcpReassemblyError::AggregateByteLimit {
                limit: self.limits.max_aggregate_bytes,
            });
        }
        ...
        if closed {
            ...
            events.push(TcpReassemblyEvent::Closed {
                flow: segment.flow,
                reset: segment.rst,
            });
        } else {
    ...
    pub fn expire(&mut self, now: Instant) -> Vec<TcpReassemblyEvent> {
        let keys = self
            .flows
            .iter()
    ...
    pub fn flush(&mut self) -> Vec<TcpReassemblyEvent> {
    ...
    fn remove_flows(&mut self, mut keys: Vec<TcpFlowKey>) -> Vec<TcpReassemblyEvent> {
        ...
        for key in keys {
            ...
            if let Some((&next, _)) = state.pending.first_key_value() {
                if next > state.next_offset {
                    events.push(TcpReassemblyEvent::Gap {
                        flow: key.clone(),
            ...
            events.push(TcpReassemblyEvent::Evicted {
                flow: key,
...
mod tests {
    ...
    fn segment(sequence: u32, payload: &'static [u8]) -> TcpSegment {
        TcpSegment {
            flow: flow(),
            sequence,
    ...
    fn out_of_order_segments_emit_in_sequence() {
        ...
        let data = events
            ...
            .filter_map(|event| match event {
                TcpReassemblyEvent::Data { bytes, .. } => Some(bytes),
                _ => None,
            })
            .collect::<Vec<_>>();
    ...
    fn retransmission_is_reported_without_duplicate_data() {
        ...
        assert!(events.iter().any(|event| matches!(
            event,
            TcpReassemblyEvent::Retransmission {
                bytes: 2,
                conflicting: false,
                ..
            }
        ...
        assert!(!events
            .iter()
            .any(|event| matches!(event, TcpReassemblyEvent::Data { .. })));
    }
    ...
    fn sequence_numbers_unwrap_across_u32_boundary() {
        ...
        assert!(events.iter().any(
            |event| matches!(event, TcpReassemblyEvent::Data { sequence: 0, bytes, .. } if bytes.as_ref() == b"cd")
        ));
    ...
    fn data_before_capture_base_is_old_not_a_four_gibibyte_gap() {
        ...
        assert!(events
            .iter()
            .any(|event| matches!(event, TcpReassemblyEvent::Retransmission { bytes: 1, .. })));
    }
    ...
    fn aggregate_limit_rejects_emitted_history_atomically() {
        ...
        assert_eq!(
            ...
            TcpReassemblyError::AggregateByteLimit { limit: 3 }
        );
        ...
        assert!(events
            .iter()
            .any(|event| matches!(event, TcpReassemblyEvent::Data { sequence: 100, .. })));
    ...
    fn pending_segment_limit_is_typed_and_atomic() {
        ...
        assert_eq!(
            ...
            TcpReassemblyError::SegmentLimit { limit: 2 }
        );
        assert_eq!(reassembler.aggregate_bytes(), 2);
        ...
        assert!(matches!(
            flushed.as_slice(),
            [
                TcpReassemblyEvent::Gap { .. },
                TcpReassemblyEvent::Evicted {
                    pending_bytes: 2,
                    ..
                }
            ]
    ...
    fn aggregate_limit_charges_sparse_segment_metadata() {
        ...
        assert_eq!(
            reassembler.push(segment(105, b"c"), now).unwrap_err(),
            TcpReassemblyError::AggregateByteLimit { limit: 130 }
        );
        assert_eq!(reassembler.aggregate_bytes(), 2);
        assert_eq!(reassembler.aggregate_memory_charge(), 130);
    }
    ...
    fn established_fin_bounds_later_data_and_conflicting_fin() {
        ...
        assert_eq!(
            ...
            TcpReassemblyError::BeyondFinalSequence { final_offset: 6 }
        );
        let mut conflicting = segment(104, b"");
        conflicting.fin = true;
        assert_eq!(
            reassembler.push(conflicting, now).unwrap_err(),
            TcpReassemblyError::ConflictingFinalSequence {
                existing_offset: 6,
                new_offset: 4
            }
        );
