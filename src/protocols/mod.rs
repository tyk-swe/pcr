...
pub use capture_link::{
    BsdLoop, BsdLoopCodec, BsdNull, BsdNullCodec, CaptureByteOrder, LinuxSll, LinuxSll2,
    LinuxSll2Codec, LinuxSllCodec,
};
pub use icmp::{Icmpv4, Icmpv4Codec, Icmpv6, Icmpv6Codec};
pub use ip::{Ipv4, Ipv4Codec, Ipv6, Ipv6Codec, RawIpCodec};
pub use ipv6_ext::{
    DestinationOptions, DestinationOptionsCodec, HopByHop, HopByHopCodec, Ipv6Fragment,
...
/// libpcap `DLT_EN10MB` / pcap `LINKTYPE_ETHERNET`.
pub const LINKTYPE_ETHERNET: u32 = 1;
/// Common BSD `DLT_RAW` value.
pub const DLT_RAW: u32 = 12;
/// pcap `LINKTYPE_RAW`, whose first nibble selects IPv4 or IPv6.
...
impl ProtocolModule for BuiltinProtocols {
    fn register(&self, builder: &mut RegistryBuilder) -> Result<(), RegistryError> {
        ...
        builder.bind_link_type(LINKTYPE_ETHERNET, "ethernet")?;
        builder.bind_link_type(0, "bsd_null")?;
        builder.bind_link_type(108, "bsd_loop")?;
        builder.bind_link_type(113, "linux_sll")?;
        builder.bind_link_type(276, "linux_sll2")?;
        builder.bind_link_type(DLT_RAW, "raw_ip")?;
        builder.bind_link_type(LINKTYPE_RAW, "raw_ip")?;
        builder.bind_link_type(LINKTYPE_IPV4, "ipv4")?;
        builder.bind_link_type(LINKTYPE_IPV6, "ipv6")?;
...
mod tests {
    ...
    fn builtin_registration_is_deterministic_and_has_portable_roots() {
        ...
        assert_eq!(
            first
                .root_for_link_type(LINKTYPE_ETHERNET)
                .unwrap()
                .as_str(),
            "ethernet"
        );
        assert_eq!(
            first.root_for_link_type(LINKTYPE_RAW).unwrap().as_str(),
            "raw_ip"
        );
    ...
    fn udp_trailer_remains_inside_ipv4_length_but_outside_udp_length() {
        ...
        let document = crate::core::PacketDocument::from_packet(&decoded.packet);
