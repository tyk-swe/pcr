...
use crate::capture::{CaptureOverflowPolicy, CaptureQueueLimits, CaptureSession, CapturedFrame};
use bytes::Bytes;
...
fn old_kernel_timestamp_maps_before_dequeue_observation() {
    ...
    let received_at = monotonic_packet_time(packet_timestamp, observed_wall, observed_at).unwrap();
