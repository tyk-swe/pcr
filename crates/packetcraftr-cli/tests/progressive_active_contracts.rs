...
impl capture::Session for FakeCapture {
    ...
    fn shutdown(&mut self) -> Result<(), packetcraftr_netio::Error> {
        ...
        if self.0.fail_shutdown_at == call {
            return Err(packetcraftr_netio::Error::Capture {
...
fn exchange_template() -> packetcraftr_core::template::Template {
    ...
    packetcraftr_core::template::Template::new(packet).axis(
        2,
        "bytes",
        vec![
...
impl Harness {
    ...
    fn parts(
        ...
        packetcraftr::policy::PolicyAuthorizer<'_>,
    ) {
        (
            Arc::clone(self.client.registry()),
            ExchangeExecutor::new(&self.client, exchange_options()),
            packetcraftr::policy::PolicyAuthorizer::for_packets(&self.policy),
        )
    }
}
...
fn exchange_events_follow_provider_confirmation_and_completion() {
    ...
    options
        .validate()
        .unwrap_or_else(|error| panic!("valid exchange options: {error}"));
...
fn blocked_encoder_does_not_hold_up_progressive_deadline_cleanup() {
    ...
    let stream = packetcraftr_cli::output::stream::StreamEncoder::new(
        packetcraftr_cli::output::contract::Command::Scan,
        Writer {
            release: wait,
            started,
            dropped,
    ...
    let error = scan::run_with_events(
        ...
        move |_| {
            callback_stream
                .emit_data(
                    TestRecord(serde_json::json!({"event": "probe"})),
                    Vec::new(),
                )
                .map_err(|error| {
