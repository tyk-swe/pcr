...
The public surface is the `packetcraftr` façade. Applications may use its
...
`packetcraftr-*` component packages are internal workspace boundaries. They
are not published independently and are not compatibility entry points.

Provider contracts are owned by `packetcraftr::io` and reexported at the crate
root. Import them from `packetcraftr` or `packetcraftr::io`.
...
`PacketDocument::parse` applies the default byte, layer, and recursive
...
- a `FailureKind` that determines the documented CLI exit-code family;
...
`api/packetcraftr-public-api.txt` records façade item paths, declarations, and
inherent or trait member signatures generated by the pinned rustdoc toolchain.
CI rebuilds and compares that baseline. Additive changes still need review;
removals, renames, field or variant changes, signature changes, trait-bound
changes, and ownership-path changes are compatibility changes.

Regenerate the baseline deliberately after reviewing a public change.
Implementation details, private platform adapters, and internal component
package paths are outside this façade promise.
