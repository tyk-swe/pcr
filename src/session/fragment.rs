...
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum FragmentReassemblyEvent {
...
pub struct FragmentReassembler {
    ...
    flows: HashMap<FragmentKey, DatagramState>,
    aggregate_bytes: usize,
    aggregate_memory_charge: usize,
}

impl FragmentReassembler {
    pub fn new(limits: ReassemblyLimits, overlap_policy: FragmentOverlapPolicy) -> Self {
        Self {
            limits,
            overlap_policy,
            flows: HashMap::new(),
    ...
    pub fn push(
        ...
        fragment: Fragment,
        now: Instant,
    ) -> Result<Option<FragmentReassemblyEvent>, FragmentError> {
        let end = fragment
            .offset
            .checked_add(
                u32::try_from(fragment.bytes.len()).map_err(|_| FragmentError::OffsetOverflow)?,
            )
            .ok_or(FragmentError::OffsetOverflow)?;
        if end as usize > self.limits.max_bytes_per_flow {
            return Err(FragmentError::FlowByteLimit {
                limit: self.limits.max_bytes_per_flow,
            });
        }
        if !self.flows.contains_key(&fragment.key) && self.flows.len() >= self.limits.max_flows {
            return Err(FragmentError::FlowLimit {
            ...
            match candidate.final_length {
                ...
                _ => {
                    ...
                    if prior_fragment_extends_past_end {
                        return Err(FragmentError::BeyondFinalLength { final_length: end });
                    }
        ...
        let aggregate =
            self.aggregate_bytes
                .checked_add(added)
                .ok_or(FragmentError::AggregateByteLimit {
                    limit: self.limits.max_aggregate_bytes,
                })?;
        if aggregate > self.limits.max_aggregate_bytes {
            return Err(FragmentError::AggregateByteLimit {
                limit: self.limits.max_aggregate_bytes,
            });
        }
        ...
        if let Some(length) = complete {
            ...
            return Ok(Some(FragmentReassemblyEvent::Complete(
                ReassembledDatagram {
                    key: fragment.key,
    ...
    pub fn expire(&mut self, now: Instant) -> Vec<FragmentReassemblyEvent> {
        ...
        expired
            ...
            .filter_map(|key| {
                ...
                Some(FragmentReassemblyEvent::Expired {
                    key,
    ...
    pub fn flush(&mut self) -> Vec<FragmentReassemblyEvent> {
...
) -> Result<(BTreeMap<u32, Bytes>, usize, bool), FragmentError> {
    ...
    for (index, value) in fragment.iter().copied().enumerate() {
        ...
        if present[position] {
            if bytes[position] != value {
                conflict = true;
                if policy == FragmentOverlapPolicy::RejectConflicting {
                    return Err(FragmentError::ConflictingOverlap {
...
mod tests {
    ...
    fn out_of_order_fragments_reassemble() {
        ...
        let mut reassembler = FragmentReassembler::new(
            ...
            FragmentOverlapPolicy::RejectConflicting,
        );
        assert!(reassembler
            .push(
                Fragment {
        ...
        assert!(matches!(
            event,
            FragmentReassemblyEvent::Complete(value) if value.bytes == Bytes::from_static(b"abcdef")
        ));
    }
    ...
    fn conflicting_overlap_rejects_by_default() {
        ...
        assert!(matches!(
            error,
            FragmentError::ConflictingOverlap { offset: 2 }
        ));
    }
    ...
    fn expiry_emits_incomplete_event_and_releases_bytes() {
        ...
        let mut reassembler =
            FragmentReassembler::new(limits, FragmentOverlapPolicy::RejectConflicting);
        reassembler
            .push(
                Fragment {
                    key: key(),
    ...
    fn aggregate_limit_charges_sparse_fragment_metadata() {
        ...
        assert_eq!(
            reassembler
                ...
                .unwrap_err(),
            FragmentError::AggregateByteLimit { limit: 193 }
        );
        assert_eq!(reassembler.aggregate_bytes(), 1);
        assert_eq!(reassembler.aggregate_memory_charge(), 193);
    }
