...
//!
//! The frame vocabulary itself belongs to [`packetcraftr_model`]. [`Frame`],
//! [`LinkType`], [`Direction`], and [`FrameError`] are re-exported here because
//! every reader, writer, and transcoder in this crate produces or consumes
//! them, and callers should not need a second import to use the capture API.
...
pub use packetcraftr_model::{Direction, Frame, FrameError, LinkType};
