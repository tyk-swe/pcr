...
use packetcraftr_capture::{Format, Frame, LinkType, Reader, Writer};
...
fn cli_errors_preserve_classification_when_crossing_a_workflow_boundary() {
    let classification = packetcraftr_model::error::Classification::new(
        "io.test_boundary",
        packetcraftr_model::error::Kind::Io,
        None,
    );
    ...
    assert_eq!(
        packetcraftr_model::error::Classified::classification(&boundary),
        classification
    );
    assert_eq!(
        packetcraftr_model::error::Classified::causes(&boundary),
        ["underlying failure"]
    );
}
