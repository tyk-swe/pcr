...
use super::{
    Frame, IoSendReport, IpAddr, Ipv4Addr, Ipv6Addr, LinkMode, LinkType, LiveIoError,
    NetworkEnvelope, ReplayError,
};
...
pub fn replay_network_envelope(frame: &Frame) -> Result<NetworkEnvelope, LiveIoError> {
...
pub(super) fn replay_wire_destinations(
    frame: &Frame,
) -> Result<ReplayWireDestinations, packetcraftr_packet::semantics::SemanticError> {
    let bytes = frame.bytes().as_ref();
    let (network_offset, protocol) = match frame.link_type {
        ...
        LinkType::ETHERNET if bytes.len() >= ETHERNET_HEADER_LEN => {
            ...
            for _ in 0..packetcraftr_packet::build::DEFAULT_MAX_LAYERS {
...
fn collect_ipv4_wire_destinations(
...
) -> Result<(), packetcraftr_packet::semantics::SemanticError> {
    ...
    for destination in
        packetcraftr_packet::semantics::ipv4_source_route_destinations(&header[20..])?
    {
            ...
            0 | 43 | 60 => {
                ...
                if next_header == 43 && extension[2] == 4 && !saw_routing_header {
                    ...
                    let valid = expected_length == Some(extension.len())
                        && packetcraftr_packet::semantics::validate_segment_route(
