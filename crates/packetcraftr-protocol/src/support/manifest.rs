...
const ALL_BUILD_VALUES: [&str; 32] = supported_protocols(Capability::Build);
const ALL_DISSECT_VALUES: [&str; 33] = supported_protocols(Capability::Dissect);
...
const LIVE_BUILD_VALUES: [&str; 28] = protocol_names([
    BuiltinProtocol::Ah,
    BuiltinProtocol::Arp,
...
mod tests {
    ...
    fn manifest_serialization_is_versioned_and_complete() {
        ...
        assert_eq!(value["protocols"].as_array().unwrap().len(), 33);
    ...
    fn catalog_membership_is_independent_of_live_backend_features() {
        ...
        assert_eq!(BuiltinProtocol::ALL.len(), 33);
