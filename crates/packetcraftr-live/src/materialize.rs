...
use packetcraftr_network::route::{Materialized as MaterializedRoute, Plan as PlannedRoute};
use packetcraftr_packet::protocol::link::Ethernet;
...
pub(super) fn materialize_link_fields(
    ...
    if route.plan.mode != packetcraftr_network::link::Mode::Layer2 {
        return Ok(false);
    }
