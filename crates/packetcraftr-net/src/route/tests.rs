...
fn fully_specified_layer2_frame_needs_no_neighbor_source() {
    ...
    let mut packet = Packet::new();
    packet
        .push(packetcraftr_protocols::link::Ethernet {
            source: [2, 0, 0, 0, 0, 1],
            destination: [2, 0, 0, 0, 0, 2],
            ..packetcraftr_protocols::link::Ethernet::default()
        })
...
fn destination_free_layer2_requires_explicit_interface() {
    let mut packet = Packet::new();
    packet.push(packetcraftr_protocols::link::Ethernet {
        source: [2, 0, 0, 0, 0, 1],
        destination: [2, 0, 0, 0, 0, 2],
        ether_type: WireValue::Exact(0x88b5),
    });
