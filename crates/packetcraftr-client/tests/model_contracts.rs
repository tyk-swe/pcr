...
use packetcraftr_client::{
    ...
    target::{Error as TargetError, Hostname, IpVersion, Resolver, Target},
};
...
fn resolved_target_selects_first_and_family_specific_addresses() {
    ...
    assert_eq!(
        resolved.address_for_version(IpVersion::V4),
        Some(resolver.0[1])
    );
    assert_eq!(
        resolved.address_for_version(IpVersion::V6),
        Some(resolver.0[0])
    );
}
...
fn client_exposes_the_exact_registry_arc() {
    let registry = Arc::new(packetcraftr_protocol::builtin::registry().expect("built-in registry"));
