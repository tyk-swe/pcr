...
use packetcraftr_packet::layer::{Layer, Malformed, Raw};
use packetcraftr_packet::{Packet, build, decode};
use packetcraftr_protocol::application::Dns;
use packetcraftr_protocol::capture::{BsdLoop, BsdNull, LinuxSll, LinuxSll2};
use packetcraftr_protocol::gre::Gre;
use packetcraftr_protocol::icmp::{Icmpv4, Icmpv6};
use packetcraftr_protocol::ipv6::{DestinationOptions, Fragment, HopByHop, SegmentRoutingHeader};
use packetcraftr_protocol::link::{Arp, Ethernet, Llc, Snap, Vlan};
use packetcraftr_protocol::network::{Ipv4, Ipv6};
use packetcraftr_protocol::transport::{Sctp, Tcp, Udp};
use packetcraftr_protocol::tunnel::{Ah, Erspan, Esp, Geneve, L2tpv3, Mpls, Ppp, Pppoe, Vxlan};
use packetcraftr_protocol::{builtin, quoted_icmp_error_kind};
...
fn response_matchers_correlate_reverse_udp_and_icmp_echo_flows() {
    ...
    assert!(
        quoted_icmp_error_kind(
            &request,
            &response,
            packetcraftr_protocol::QuotedProbeTransport::Udp,
        )
        .is_none()
    );
}
