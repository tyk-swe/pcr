...
use packetcraftr_capture as capture;
use packetcraftr_capture::{Frame, Limits, PcapNgOptions, PcapOptions, Writer};
use packetcraftr_client as client;
use packetcraftr_net as net;
use packetcraftr_net::capture::Provider as _;
...
impl From<&policy::Policy> for CaptureBudget {
...
pub(crate) fn run_capture(
    ...
    match output {
        output::contract::Format::Text => {
            let capture = net_native::capture::SystemProvider
        ...
        output::contract::Format::Pcap | output::contract::Format::Pcapng => {
            ...
            let mut capture = net_native::capture::SystemProvider
                .arm_capture(&route, limits)
                .map_err(CliError::classified)?;
    ...
    F: FnMut(Frame, u64) -> Result<(), CliError>,
{
    ...
    while frames < budget.max_frames {
        ...
        if next_bytes > budget.max_bytes {
            let error = CliError::classified(policy::Error::ByteLimit {
                actual: next_bytes,
