...
use packetcraftr_model::{DEFAULT_MAX_FRAME_BYTES, Frame as CaptureFrame};
...
impl Default for Limits {
    fn default() -> Self {
        Self {
            ...
            snap_length: DEFAULT_MAX_FRAME_BYTES,
...
impl Limits {
    ...
    pub fn validate(self) -> Result<Self, Error> {
        ...
        for (field, value, maximum) in [
            ...
            ("snap_length", self.snap_length, DEFAULT_MAX_FRAME_BYTES),
        ] {
            if value > maximum {
                return Err(Error::InvalidCaptureQueueLimit {
                    field,
...
impl SystemSession {
    /// Wraps a provider-supplied session so every blocking wait is checked
    /// against [`MAX_TIMEOUT`] regardless of which backend produced it.
