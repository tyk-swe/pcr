...
- Protocol details list registered display-filter aliases, either-endpoint
  ...
  with `filter_fields` JSON metadata separate from parent bindings. Rust
...
- NDJSON output bounds each write and flush to one second, including terminal
  records. Blocked output fails as an incomplete stream without retrying stdout
  during cleanup or process shutdown; workflow publication deadlines stay bounded.
