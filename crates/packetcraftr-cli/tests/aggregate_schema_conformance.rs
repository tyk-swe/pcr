        ...
        validator.validate(&document).unwrap_or_else(|error| {
            panic!(
                ...
                serde_json::to_string_pretty(&document)
                    .unwrap_or_else(|error| panic!("document renders: {error}"))
            )
        });
    }
}
...
fn analysis_stats_report() -> packetcraftr_core::analysis::stats::Report {
    ...
    use packetcraftr_core::analysis::{IpCounters, IpDatagramOutcome, IpFamilyCounters};
...
fn expert_case() -> Value {
    ...
    use packetcraftr_core::analysis::expert::{Finding as AnalysisFinding, Summary};
...
#[test]
fn filter_discovery_metadata_is_required_and_validates_all_binding_kinds() {
    let validator = schema_validator();
    for protocol in [
        ...
        validator
            .validate(&document)
            .unwrap_or_else(|error| panic!("registry metadata validates: {error}"));
