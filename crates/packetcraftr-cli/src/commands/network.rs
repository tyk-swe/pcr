...
use packetcraftr_net::{interface::Provider as _, route::Provider as _};
...
pub(crate) fn run_routes(output: output::contract::Format) -> Result<(), CliError> {
    let interfaces = net_native::interface::SystemProvider
        .interfaces()
        .map_err(CliError::classified)?;
    let provider = net_native::route::SystemProvider;
