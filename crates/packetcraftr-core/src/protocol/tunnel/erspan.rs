...
reflective_layer! {
    ...
    impl Erspan {
        "version" => { kind: Unsigned, tier: Required, max: 15_u64, description: "Header version: 1 is Type II, 2 is Type III", get |layer| Some(reflect_get(&layer.version)), set |layer, value, name| { reflect_set_bounded(&mut layer.version, erspan_schema(), name, value, 0xf_u64)?; if layer.version == 2 && layer.type3.is_none() { layer.type3 = Some(ErspanType3::default()); } Ok(()) }, layout: (0, 2) },
        "vlan" => { kind: Unsigned, tier: Optional, default: "0", description: "VLAN of the mirrored frame", reflect_bounded: vlan, 0xfff_u64, layout: (0, 2) },
        "cos" => { kind: Unsigned, tier: Optional, default: "0", description: "Class of service of the mirrored frame", reflect_bounded: cos, 7_u64, layout: (2, 4) },
        "encapsulation" => { kind: Unsigned, tier: Optional, default: "0", description: "Type II trunk encapsulation; Type III bad/short frame bits", reflect_bounded: encapsulation, 3_u64, layout: (2, 4) },
        "truncated" => { kind: Bool, tier: Optional, default: "false", description: "The mirrored frame was truncated by the session MTU", reflect: truncated, layout: (2, 4) },
        "session_id" => { kind: Unsigned, tier: Required, description: "10-bit monitoring-session identifier", reflect_bounded: session_id, 0x3ff_u64, layout: (2, 4) },
        "index_word" => { kind: Unsigned, tier: Optional, default: "0", description: "Type II reserved bits and port index, packed as the final word", reflect: index_word, layout: (4, 8) },
        "timestamp" => { kind: Unsigned, tier: Optional, max: 4294967295_u64, description: "Type III wire-format timestamp", get |layer| layer.type3.as_ref().map(|type3| FieldValue::from(type3.timestamp)), set |layer, value, name| match value { FieldValue::Unsigned(value) => { layer.type3.get_or_insert_with(ErspanType3::default).timestamp = u32::try_from(value).map_err(|_| out_of_range(erspan_schema(), name))?; Ok(()) }, _ => Err(wrong_type(erspan_schema(), name, "unsigned")) } },
        "sgt" => { kind: Unsigned, tier: Optional, max: 65535_u64, description: "Type III security group tag", get |layer| layer.type3.as_ref().map(|type3| FieldValue::from(type3.sgt)), set |layer, value, name| match value { FieldValue::Unsigned(value) => { layer.type3.get_or_insert_with(ErspanType3::default).sgt = u16::try_from(value).map_err(|_| out_of_range(erspan_schema(), name))?; Ok(()) }, _ => Err(wrong_type(erspan_schema(), name, "unsigned")) } },
        "flags" => { kind: Unsigned, tier: Optional, max: 65535_u64, description: "Type III flag half-word: P bit, frame type, hardware ID, direction, granularity, and the subheader O bit", get |layer| layer.type3.as_ref().map(|type3| FieldValue::from(type3.flags)), set |layer, value, name| match value { FieldValue::Unsigned(value) => { layer.type3.get_or_insert_with(ErspanType3::default).flags = u16::try_from(value).map_err(|_| out_of_range(erspan_schema(), name))?; Ok(()) }, _ => Err(wrong_type(erspan_schema(), name, "unsigned")) } },
        "subheader" => { kind: Bytes, tier: Optional, description: "Type III 8-byte platform-specific subheader", get |layer| layer.type3.as_ref().and_then(|type3| type3.subheader.clone()).map(FieldValue::Bytes), set |layer, value, name| match value { FieldValue::Bytes(value) => { layer.type3.get_or_insert_with(ErspanType3::default).subheader = Some(value); Ok(()) }, _ => Err(wrong_type(erspan_schema(), name, "bytes")) } }
    }
