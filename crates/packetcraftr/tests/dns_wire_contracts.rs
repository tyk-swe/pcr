...
fn names_are_lossless_case_insensitive_and_safely_presented() {
    ...
    assert!(Name::from_labels([Bytes::new()]).is_err());
    assert!(Name::from_labels([Bytes::from(vec![b'a'; 64])]).is_err());
    assert!(Name::from_labels((0..4).map(|_| Bytes::from(vec![b'a'; 63]))).is_err());
