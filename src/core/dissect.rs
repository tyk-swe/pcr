...
impl Dissector {
    ...
    pub fn decode_with_root(
        ...
        let frame = CapturedFrame::new(
            ...
            super::capture::LinkType(u32::MAX),
            bytes.clone(),
        )?;
        if options.max_layers == 0 {
            return Err(DecodeError::LayerLimit { limit: 0 });
...
fn raw_decoded_frame(frame: CapturedFrame, diagnostic: Diagnostic) -> DecodedPacket {
    let original = frame.bytes.clone();
    let mut packet = Packet::new();
...
mod tests {
    ...
    use crate::core::{
        ...
        LayerDecodeContext, LayerEncodeContext, LayerSchema, LinkType, RegistryBuilder,
    };
    ...
    fn zero_layer_limit_applies_to_unknown_link_types() {
        let frame =
            CapturedFrame::new(std::time::SystemTime::UNIX_EPOCH, LinkType(9999), vec![1]).unwrap();
        assert!(matches!(
