...
mod tests {
    ...
    #[test]
    fn normalise_label_behavior() {
        assert_eq!(normalise_label(""), "unknown");
        assert_eq!(normalise_label("tcp"), "tcp");
        assert_eq!(normalise_label("layer2"), "layer2");
        assert_eq!(normalise_label("ethernet"), "ethernet");
        assert_eq!(normalise_label("ipv4"), "ipv4");
        assert_eq!(normalise_label("test"), "test");
    }

    #[test]
    #[serial]
    fn record_listener_dropped_packet_increments_counter() {
        LISTENER_DROPPED_PACKETS.reset();

        record_listener_dropped_packet("queue_full");

        let value = LISTENER_DROPPED_PACKETS
    ...
    #[test]
    #[serial]
    fn record_rule_action_increments_counter() {
        RULE_ACTIONS.reset();

        record_rule_action("command", "queued");

        let value = RULE_ACTIONS.with_label_values(&["command", "queued"]).get();
        assert_eq!(value, 1);
    }

    #[test]
    #[serial]
    fn record_rule_executor_drop_increments_counter() {
        RULE_ACTION_DROPS.reset();

        record_rule_executor_drop("send", "queue_full");

        let value = RULE_ACTION_DROPS
            .with_label_values(&["send", "queue_full"])
            .get();
        assert_eq!(value, 1);
    }

    #[test]
    fn spawn_prometheus_exporter_validation() {
        let runtime = tokio::runtime::Runtime::new().unwrap();
        let handle = runtime.handle().clone();

        let cases = vec!["invalid_address", "127.0.0.1:99999", ""];

        for case in cases {
            let result = spawn_prometheus_exporter(&handle, case);
            assert!(
                matches!(result, Err(TelemetryError::ParseAddress { .. })),
                "Expected ParseAddress error for input '{}', got {:?}",
                case,
                result
            );
        }
    ...
    #[test]
    #[serial]
    fn record_frame_sent_increments_counters() {
        FRAMES_SENT.reset();
        BYTES_SENT.reset();

        record_frame_sent("ethernet", "tcp", 1500);

        let frames = FRAMES_SENT.with_label_values(&["ethernet", "tcp"]).get();
        let bytes = BYTES_SENT.with_label_values(&["ethernet", "tcp"]).get();

        assert_eq!(frames, 1, "expected frame counter to increment");
        assert_eq!(bytes, 1500, "expected bytes counter to add payload size");
    }

    #[test]
    #[serial]
    fn record_listener_packet_increments_counter() {
        LISTENER_PACKETS.reset();

        record_listener_packet("tcp");
        record_listener_packet("tcp");

        let packets = LISTENER_PACKETS.with_label_values(&["tcp"]).get();
        assert_eq!(
            packets, 2,
            "expected listener counter to reflect increments"
        );
    }

    #[test]
    #[serial]
    fn get_frame_sent_counters_returns_correct_counters() {
        FRAMES_SENT.reset();
        BYTES_SENT.reset();

        let (frames, bytes) = get_frame_sent_counters("ethernet", "tcp");
        frames.inc();
        bytes.inc_by(100);

        let frames_val = FRAMES_SENT.with_label_values(&["ethernet", "tcp"]).get();
        let bytes_val = BYTES_SENT.with_label_values(&["ethernet", "tcp"]).get();

        assert_eq!(frames_val, 1);
        assert_eq!(bytes_val, 100);
    }
}
