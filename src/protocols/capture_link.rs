...
pub struct BsdNullCodec;

#[derive(Clone, Copy, Debug, Default)]
pub struct BsdLoopCodec;
...
#[derive(Clone, Copy, Debug, Default)]
pub struct LinuxSllCodec;
#[derive(Clone, Copy, Debug, Default)]
pub struct LinuxSll2Codec;
...
fn sll_layout(v2: bool) -> Vec<crate::core::FieldLayout> {
    if v2 {
        vec![
...
mod tests {
    ...
    fn null_and_loop_endianness_decode_to_ipv4() {
        ...
        for (link_type, family) in [
            ...
            let decoded = Dissector::new(Arc::clone(&registry))
                .decode(
                    CapturedFrame::new(SystemTime::UNIX_EPOCH, link_type, frame).unwrap(),
    ...
    fn sll_and_sll2_use_their_protocol_offsets() {
        ...
        let first = Dissector::new(Arc::clone(&registry))
            .decode(
                CapturedFrame::new(SystemTime::UNIX_EPOCH, LinkType::LINUX_SLL, sll).unwrap(),
                DecodeOptions::default(),
            )
            .unwrap();
        let second = Dissector::new(registry)
            .decode(
                CapturedFrame::new(SystemTime::UNIX_EPOCH, LinkType::LINUX_SLL2, sll2).unwrap(),
                DecodeOptions::default(),
            )
            .unwrap();
