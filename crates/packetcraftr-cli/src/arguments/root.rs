...
An aggregate JSON document is built in memory before it is written, so reading a whole capture that
way costs several times what the input holds. NDJSON reports the same records in constant memory;
prefer it, or lower --max-bytes, for a large capture.
