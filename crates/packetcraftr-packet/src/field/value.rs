...
///
/// The set is deliberately closed: every codec, renderer, mutator, and output
/// mapping in the workspace translates it exhaustively, so adding a kind must
/// be a compile error at each of those sites rather than a silent fallback.
...
///
/// Closed for the same reason as [`FieldKind`]: it is the paired value union.
