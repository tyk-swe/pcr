...
use super::super::super::common::{
    ensure_encode_budget, invalid, protocol, text_list, unsigned_list, wrong_layer,
};
...
fn read_only(field: &str) -> Result<(), FieldError> {
    Err(FieldError::ReadOnly {
        protocol: tls_schema().protocol.clone(),
...
reflective_layer! {
    ...
    impl Tls {
        ...
        "handshake_type" => { kind: Unsigned, tier: Optional, max: 255_u64, description: "Handshake message type, when the whole message is in this segment", get |layer| layer.handshake_type.map(FieldValue::from), set |_layer, _value, name| read_only(name) },
        "cipher_suite" => { kind: Unsigned, tier: Optional, max: 65535_u64, description: "Cipher suite selected by a ServerHello", get |layer| layer.cipher_suite.map(FieldValue::from), set |_layer, _value, name| read_only(name) },
        "selected_version" => { kind: Unsigned, tier: Optional, max: 65535_u64, description: "Version selected by a ServerHello", get |layer| layer.selected_version.map(FieldValue::from), set |_layer, _value, name| read_only(name) },
        "key_share_group" => { kind: Unsigned, tier: Optional, max: 65535_u64, description: "Named group of a ServerHello key share", get |layer| layer.key_share_group.map(FieldValue::from), set |_layer, _value, name| read_only(name) },
        ...
        "sni" => { kind: Text, tier: Optional, description: "Validated server name offered by a ClientHello", get |layer| layer.sni.clone().map(FieldValue::Text), set |_layer, _value, name| read_only(name) },
        "sni_raw" => { kind: Text, tier: Optional, description: "Verbatim server name bytes in hexadecimal", get |layer| layer.sni_raw.as_ref().map(|raw| FieldValue::Text(hex(raw))), set |_layer, _value, name| read_only(name) },
        "ja3" => { kind: Text, tier: Optional, description: "Advisory JA3 fingerprint of a ClientHello (MD5 digest)", get |layer| layer.ja3.clone().map(FieldValue::Text), set |_layer, _value, name| read_only(name) },
        "ja3_raw" => { kind: Text, tier: Optional, description: "Advisory JA3 fingerprint of a ClientHello before hashing", get |layer| layer.ja3_raw.clone().map(FieldValue::Text), set |_layer, _value, name| read_only(name) },
        "ja4" => { kind: Text, tier: Optional, description: "Advisory JA4 fingerprint of a ClientHello", get |layer| layer.ja4.clone().map(FieldValue::Text), set |_layer, _value, name| read_only(name) },
        "alpn" => { kind: List, element: Text, tier: Optional, description: "Application protocols offered or selected", get |layer| optional_list(&layer.alpn), set |_layer, _value, name| read_only(name) },
        "cipher_suites" => { kind: List, element: Unsigned, tier: Optional, description: "Cipher suites offered by a ClientHello", get |layer| optional_codes(&layer.cipher_suites), set |_layer, _value, name| read_only(name) },
        "supported_versions" => { kind: List, element: Unsigned, tier: Optional, description: "Versions offered by a ClientHello", get |layer| optional_codes(&layer.supported_versions), set |_layer, _value, name| read_only(name) },
        "supported_groups" => { kind: List, element: Unsigned, tier: Optional, description: "Named groups offered by a ClientHello", get |layer| optional_codes(&layer.supported_groups), set |_layer, _value, name| read_only(name) }
    }
