...
pub struct HopByHopCodec;

#[derive(Clone, Copy, Debug, Default)]
pub struct DestinationOptionsCodec;
    ...
    fn encode(
            ...
            })
        {
            strict_or_diagnostic(
                ...
                "fragment_offset",
                "fragment payload must be Raw; fragment typed packets with an explicit PacketTransform",
                context,
                &mut diagnostics,
            )?;
        }
...
#[derive(Clone, Copy, Debug, Default)]
pub struct SegmentRoutingHeaderCodec;
...
impl LayerCodec for SegmentRoutingHeaderCodec {
    ...
    fn encode(
        ...
        if segments_left > expected_last {
            ...
            diagnostics.push(
                crate::core::Diagnostic::warning("build.srh_segments_left", message)
...
mod tests {
    ...
    use super::*;
    use crate::core::{BuildContext, BuildOptions, Builder, DecodeOptions, Dissector, Packet};
    use crate::protocols::{default_registry, Ipv6, Udp};
