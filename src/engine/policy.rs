...
use crate::engine::request::{PacketRequest, TransmissionRequest};
use crate::engine::spec::{Ipv6ExtHeader, PacketSpec, TargetAddress, TransportSpec};
...
mod tests {
    use super::*;
    use crate::engine::request::{FragmentProfile, FragmentRequest, IpRequest};
    use crate::engine::spec::{DestinationSpec, Ipv6Spec};

    #[test]
    fn classifies_local_private_documentation_and_public_targets() {
        assert_eq!(
        ...
        assert_eq!(classify_ip("8.8.8.8".parse().unwrap()), TargetScope::Public);
        assert_eq!(classify_ip("::1".parse().unwrap()), TargetScope::Local);
        assert_eq!(
    ...
    #[test]
    fn rejects_public_target_without_opt_in() {
        let plan = TrafficPlan::new(TrafficMode::Send, TargetScope::Public);
        let err = TrafficPolicy::default()
            .authorize(&plan)
            .expect_err("public target should fail closed");

        assert_eq!(err.code, PolicyRejectionCode::PublicTarget);
    }

    #[test]
    fn allows_public_target_with_opt_in() {
        let plan = TrafficPlan::new(TrafficMode::Send, TargetScope::Public);
        ...
        assert_eq!(
            policy.authorize(&plan).expect("policy outcome").status,
            "allowed"
        );
    }

    #[test]
    fn rejects_malformed_without_opt_in() {
        let mut plan = TrafficPlan::new(TrafficMode::Send, TargetScope::Local);
        plan.malformed = true;

        let err = TrafficPolicy::default()
            .authorize(&plan)
            .expect_err("malformed traffic should fail closed");

        assert_eq!(err.code, PolicyRejectionCode::MalformedRequiresOptIn);
    }

    #[test]
    fn rejects_high_volume_before_caps_without_opt_in() {
        let mut plan = TrafficPlan::new(TrafficMode::Scan, TargetScope::Private);
        plan.port_count = DEFAULT_MAX_PORTS + 1;
        plan.estimated_packets = Some((DEFAULT_MAX_PORTS + 1) as u64);

        let err = TrafficPolicy::default()
            .authorize(&plan)
            .expect_err("high volume should require opt-in");

        assert_eq!(err.code, PolicyRejectionCode::HighVolumeRequiresOptIn);
    }

    #[test]
    fn enforces_caps_after_high_volume_opt_in() {
        let mut plan = TrafficPlan::new(TrafficMode::Scan, TargetScope::Private);
        plan.port_count = DEFAULT_MAX_PORTS + 1;
        plan.estimated_packets = Some((DEFAULT_MAX_PORTS + 1) as u64);
        let policy = TrafficPolicy {
        ...
        let err = policy
            .authorize(&plan)
            .expect_err("configured cap should still apply");

        assert_eq!(err.code, PolicyRejectionCode::PortCapExceeded);
    }

    #[test]
    fn rejects_batch_and_rate_caps() {
        let policy = TrafficPolicy {
            allow_high_volume: true,
            ..Default::default()
        };

        let mut batch = TrafficPlan::new(TrafficMode::Scan, TargetScope::Local);
        batch.batch_size = DEFAULT_MAX_BATCH_SIZE + 1;
        let err = policy.authorize(&batch).expect_err("batch cap");
        assert_eq!(err.code, PolicyRejectionCode::BatchCapExceeded);

        let mut rate = TrafficPlan::new(TrafficMode::Scan, TargetScope::Local);
        rate.rate_per_sec = Some(DEFAULT_MAX_RATE_PER_SEC + 1);
        let err = policy.authorize(&rate).expect_err("rate cap");
        assert_eq!(err.code, PolicyRejectionCode::RateCapExceeded);
    }

    #[test]
    fn rejects_unbounded_without_opt_in() {
        let mut plan = TrafficPlan::new(TrafficMode::Send, TargetScope::Local);
        plan.unbounded = true;
        ...
        let err = TrafficPolicy::default()
            .authorize(&plan)
            .expect_err("unbounded should require opt-in");

        assert_eq!(err.code, PolicyRejectionCode::UnboundedSend);
    }

    #[test]
    fn rejects_zero_counts() {
        let mut plan = TrafficPlan::new(TrafficMode::Scan, TargetScope::Local);
        plan.target_count = 0;

        let err = TrafficPolicy::default()
            .authorize(&plan)
            .expect_err("zero targets should fail");

        assert_eq!(err.code, PolicyRejectionCode::CountMustBePositive);
    }

    #[test]
    fn configuration_requires_high_volume_for_raised_defaults() {
        let mut policy = TrafficPolicy::default();
        policy.budget.max_targets = DEFAULT_MAX_TARGETS + 1;

        let err = policy
            .validate_configuration()
            .expect_err("raised cap should need high-volume opt-in");

        assert_eq!(err.code, PolicyRejectionCode::HighVolumeRequiresOptIn);
    }

    #[test]
    fn request_plan_detects_malformed_fragment_profile() {
    ...
    #[test]
    fn packet_spec_target_scope_includes_ipv6_routing_final_destination() {
        let spec = PacketSpec {
            target: DestinationSpec {
                address: Some(TargetAddress::Ip("2606:4700:4700::1111".parse().unwrap())),
                interface: None,
            },
            ipv6: Ipv6Spec {
                exthdrs: vec![Ipv6ExtHeader::Routing {
                    routing_type: 0,
                    segments: vec!["fc00::1".parse().unwrap()],
                    data: None,
                }],
            },
            ..Default::default()
