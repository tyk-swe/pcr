...
use packetcraftr_cli::output::interfaces;
...
fn command_format_matrix_display_and_errors_cover_the_full_vocabulary() {
        ...
        .zip(["text", "json", "ndjson", "hex", "raw", "pcap", "pcapng"])
    {
        ...
        assert_eq!(
            <Format as clap::ValueEnum>::from_str(expected, false).unwrap(),
            format
        );
    }
...
fn interface_capability_outputs_are_stable() {
    for (capability, expected) in [
        (
            Capability::Layer2,
            packetcraftr_netio::link::Capability::Layer2,
        ),
        (
            Capability::Layer3,
            packetcraftr_netio::link::Capability::Layer3,
        ),
        (
            Capability::Layer2AndLayer3,
            packetcraftr_netio::link::Capability::Layer2AndLayer3,
        ),
    ] {
...
fn route_enum_outputs_are_stable() {
    for (mode, expected) in [
        (LinkMode::Auto, packetcraftr_netio::link::Mode::Auto),
        (LinkMode::Layer2, packetcraftr_netio::link::Mode::Layer2),
        (LinkMode::Layer3, packetcraftr_netio::link::Mode::Layer3),
    ] {
        assert_eq!(mode, expected);
    }
    for (scope, expected) in [
        (Scope::Host, packetcraftr_netio::route::Scope::Host),
        (Scope::Link, packetcraftr_netio::route::Scope::Link),
        (Scope::Private, packetcraftr_netio::route::Scope::Private),
        (Scope::Global, packetcraftr_netio::route::Scope::Global),
        (
            Scope::Multicast,
            packetcraftr_netio::route::Scope::Multicast,
        ),
        (
            Scope::Unspecified,
            packetcraftr_netio::route::Scope::Unspecified,
        ),
    ] {
    ...
    for (reason, expected) in [
        (
            ...
            packetcraftr_netio::route::SelectionReason::Local,
        ),
        (
            SelectionReason::OnLink,
            packetcraftr_netio::route::SelectionReason::OnLink,
        ),
        (
            SelectionReason::Broadcast,
            packetcraftr_netio::route::SelectionReason::Broadcast,
        ),
        (
            SelectionReason::Gateway,
            packetcraftr_netio::route::SelectionReason::Gateway,
        ),
        (
            SelectionReason::InterfaceOnly,
            packetcraftr_netio::route::SelectionReason::InterfaceOnly,
        ),
    ] {
        assert_eq!(reason, expected);
    }
    assert_eq!(
        serde_json::to_value(packetcraftr_netio::route::SelectionReason::Broadcast)
    ...
    for (kind, expected) in [
        (
            VlanKind::Ieee8021Q,
            packetcraftr_core::packet::link::VlanKind::Ieee8021Q,
        ),
        (
            VlanKind::Ieee8021Ad,
            packetcraftr_core::packet::link::VlanKind::Ieee8021Ad,
        ),
    ] {
...
fn planned_route_output_preserves_link_metadata() {
    ...
    assert_eq!(source_mac.to_string(), "00:01:02:03:04:05");
