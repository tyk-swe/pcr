...
- Every provider call that can block takes the caller's core
  ...
  `tcp::start_connect`, and `capture::group::Group::{arm, wait_ready,
  next_record}`. `capture::Cancellable` is removed: native capture waits honor
  the deadline's cancellation themselves. `packetcraftr::route::plan`,
