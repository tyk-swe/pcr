...
//! Capture-ready exchange lifecycle of the client-backed workflow executors.
//!
//! Every case drives a real [`Client`] over controlled route, neighbor,
//! transmission, and capture providers, so capture arming, shutdown, and
//! pre-send validation are observed exactly as production composes them.
...
use packetcraftr_workflow_client::dns::ClientExecutor as DnsClientExecutor;
use packetcraftr_workflow_client::scan::ClientExecutor as ScanClientExecutor;
use packetcraftr_workflow_client::traceroute::ClientExecutor as TracerouteClientExecutor;
...
fn lifecycle_exchange_options() -> ExchangeOptions {
    let mut options = ExchangeOptions {
        send: packetcraftr_client::send::Options {
            destination: None,
            plan: PlanOptions {
                link_mode: LinkMode::Layer3,
                ..PlanOptions::default()
            },
            ..packetcraftr_client::send::Options::default()
        },
...
fn client_scan_executor_waits_for_capture_and_always_shuts_it_down() {
    for fail_send in [false, true] {
        ...
        let mut executor = ScanClientExecutor::new(&client, lifecycle_exchange_options());
        let batch = ScanBatch {
...
fn client_traceroute_executor_waits_for_capture_and_always_shuts_it_down() {
    use packetcraftr_workflow::traceroute::{
